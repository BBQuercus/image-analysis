...
class interaction_factor:
    ...
    def __init__(self, rgb_mask_image, roi_mask=[], intensity_image=[]):
        ...
        for i in range(3):
            ...
            self.cluster_props.append(measure.regionprops(labeled_clusters, coordinates='xy'))
        ...
        if(roi_mask == []):
            ...
            labeled_clusters, num_clusters = ndimage.label(self.roi_mask, 
                                                           structure=self.labeling_structure)
            props = measure.regionprops(labeled_clusters, coordinates='xy')
    ...
    def calculate_IF(self):
        ...
        for sim_i in range(self.num_random_sims):
            ...
            if self.num_clusters[self.ref_color] != 0:
            ...
            if(sim_i == 0 and
               self.num_clusters[self.ref_color] != 0):
        ...
        return (calcIF, p_val, orig_meas, x, y)
        ...
        for region in self.cluster_props[self.ref_color]:
            ...
            if(self.rotate_ref):
                ...
                if(len(region.image) >=2 and len(region.image[0]) >= 2):
                    ...
                    p = measure.regionprops(l, coordinates='xy')
